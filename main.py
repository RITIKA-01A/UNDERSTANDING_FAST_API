...
@app.get("/about")
def about():
    return {'message':'A fully functional API to manage ypur patient records'}
...
def view_patient(patient_id : str = Path(..., description='ID of the pattient in the DB' , example='P001')):
    data = load_data()
...
def create_patient(patient: Patient):
    ...
    ## new pateint added to the database
...
def update_patient(patient_id:str , patient_update:PatientUpdate):
    ...
    if patient_id not in data:
        raise HTTPException(status_code=404 , detail='the id does not exist in the db')
